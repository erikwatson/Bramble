...
export function rotate(ctx: CanvasRenderingContext2D, drawingOperations: () => void, rotation: number, around: Point) {
  // freshContext(ctx, () => {
...
export function transform(ctx: CanvasRenderingContext2D, drawingOperations: () => void, options: TransformOptions = defaultTransform) {
  ...
  scale(ctx, () => {
    rotate(ctx, drawingOperations, options.angle, options.around)
